-- Two things a single price list cannot show, but several price lists together
-- can.
--
-- 1. Sold by the piece or by the kilogram. A product without a package size is
--    sold loose, and chains price it per piece or per kilogram: Europrom
--    "CELER" 109,90 a piece, Univerexport "CELER" 249,99 a kilogram. As one
--    product the app compared a piece with a kilogram.
--    The unit column alone cannot tell them apart. Maxi writes "kg" for "Mango
--    komad" (219,99, IDEA sells it for 199,99 a piece) and Veropoulos "KOM"
--    for ham that other chains price per kilogram at the same price. Keyed by
--    that column, 27 products would split, about half of them wrongly. So a
--    product is split by how it is sold only when its chains label it both
--    ways and the two groups' prices are at least twice apart, which is what a
--    piece against a kilogram looks like. A name with a number in it is a
--    package the parser could not read ("100/1") and is left alone.
--
-- 2. A price stated on another basis. Some chains publish the price of one
--    tablet, of one kilogram of a 25 kg bag or of one bottle of a case:
--    Europrom "FAIRY AUTO 84/1" 24,90 against 2.148-3.332 elsewhere, "SECER
--    25kg" 89,90 against 2.283-2.500. Such a price won every comparison.
--    Where at least three chains price a product, an offer below half of its
--    typical price (the median of each chain's median price) is marked to be
--    checked: the product screen lists it last with a note, and a shopping
--    plan never uses it. Offers between half and 60% of the typical price are
--    mostly real promotions (Lidl tuna salad 129,99 against 170-277, METRO rosé
--    1.140 against 1.925), so that band is not marked.

ALTER TABLE app.retailer_product
    ADD COLUMN sale_unit VARCHAR(3);

ALTER TABLE app.retailer_product
    ADD CONSTRAINT chk_retailer_product_sale_unit
        CHECK (sale_unit IS NULL OR sale_unit IN ('kg', 'kom'));

-- Runs after every import, for all chains at once: whether a product needs
-- splitting depends on the other chains' labels and prices.
CREATE OR REPLACE FUNCTION app.refresh_sale_units()
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
    DROP TABLE IF EXISTS pg_temp.sale_unit_product;
    DROP TABLE IF EXISTS pg_temp.sale_unit_split;

    -- The name and brand the family key reads (V71), for products without a
    -- size. One barcode is one product, so its chains' labels are outvoted
    -- together; a tie counts as sold by the piece.
    CREATE TEMP TABLE sale_unit_product ON COMMIT DROP AS
    SELECT source.id,
           source.retailer_id,
           ...
           CASE
               WHEN COUNT(*) FILTER (WHERE source.by_weight) OVER same_product
                    > COUNT(*) FILTER (WHERE NOT source.by_weight) OVER same_product
                   THEN 'kg'
               ELSE 'kom'
           END AS labelled_unit
    FROM (
        SELECT product.id,
               product.retailer_id,
               ...
               CASE
                   WHEN weight.loose
                       THEN REGEXP_REPLACE(named.name, '\m1\s*(kg|l)\M', ' ', 'gi')
                   ELSE named.name
               END AS key_name,
               brand.normalized_name AS brand_name,
               COALESCE(
                   weight.loose
                   OR UPPER(BTRIM(product.unit)) IN ('KG', 'KGR', 'G', 'GR', 'GRAM'),
                   FALSE
               ) AS by_weight
        ...
        LEFT JOIN app.brand AS brand
          ON brand.id = CASE WHEN canonical.id IS NULL
                        ...
                        END
        CROSS JOIN LATERAL (
            SELECT COALESCE(canonical.name, product.name) AS name,
        ...
        CROSS JOIN LATERAL (
            SELECT COALESCE(
                       named.name ~* '\mrinfuz\M'
                       AND named.quantity_value = 1000
                       AND named.base_unit IN ('g', 'ml'),
                       FALSE
                   ) AS loose
        ) AS weight
        WHERE NULLIF(BTRIM(named.normalized_name), '') IS NOT NULL
          AND (weight.loose OR named.quantity_value IS NULL)
    ) AS source
    WHERE source.key_name !~ '[0-9]'
    WINDOW same_product AS (
        PARTITION BY COALESCE(source.canonical_product_id, -source.id)
    );

    ANALYZE sale_unit_product;

    -- Split only where both labels are in use and the prices agree with them:
    -- the median chain price of one group at least twice the other's.
    CREATE TEMP TABLE sale_unit_split ON COMMIT DROP AS
    SELECT unit_price.base_key
    FROM (
        SELECT chain_price.base_key,
               chain_price.labelled_unit,
               PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY chain_price.price)
                   AS price
        FROM (
            SELECT sold.base_key,
                   sold.labelled_unit,
                   sold.retailer_id,
            ...
            FROM sale_unit_product AS sold
            JOIN app.current_price_offer AS offer
              ON offer.retailer_product_id = sold.id
            WHERE COALESCE(NULLIF(offer.regular_price, 0), offer.discounted_price) > 0
            GROUP BY sold.base_key, sold.labelled_unit, sold.retailer_id
        ) AS chain_price
        GROUP BY chain_price.base_key, chain_price.labelled_unit
    ) AS unit_price
    GROUP BY unit_price.base_key
    HAVING COUNT(*) = 2
       AND MAX(unit_price.price) >= 2 * MIN(unit_price.price);

    ANALYZE sale_unit_split;

    UPDATE app.retailer_product AS product
    SET sale_unit = decided.sale_unit
    FROM (
        SELECT candidate.id,
               CASE WHEN split.base_key IS NOT NULL
                    THEN sold.labelled_unit
               END AS sale_unit
        FROM app.retailer_product AS candidate
        LEFT JOIN sale_unit_product AS sold
          ON sold.id = candidate.id
        LEFT JOIN sale_unit_split AS split
          ON split.base_key = sold.base_key
        WHERE candidate.sale_unit IS NOT NULL
           OR split.base_key IS NOT NULL
    ) AS decided
    WHERE decided.id = product.id
      AND product.sale_unit IS DISTINCT FROM decided.sale_unit;
END;
$$;
...
CREATE TABLE app.product_family_typical_price (
    product_family_id BIGINT NOT NULL,
    typical_price NUMERIC(12,2) NOT NULL,
    retailer_count INTEGER NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    CONSTRAINT product_family_typical_price_pkey
        PRIMARY KEY (product_family_id),
    CONSTRAINT fk_product_family_typical_price_family
        FOREIGN KEY (product_family_id)
        REFERENCES app.product_family (id)
        ON DELETE CASCADE,
    CONSTRAINT chk_product_family_typical_price
        CHECK (typical_price > 0 AND retailer_count >= 3)
);

-- Rebuilt after every import for all chains: one chain's new prices move the
-- typical price that every other chain's offers are compared with.
...
-- The one place that decides whether an offer is too cheap to be believed.
-- The regular price is compared, so a real temporary discount is never marked.
CREATE OR REPLACE FUNCTION app.price_needs_check(
...
AS $$
    SELECT COALESCE(
        COALESCE(NULLIF(regular_price, 0), discounted_price) < typical_price * 0.5,
        FALSE
    )
$$;

SELECT app.refresh_sale_units();
