-- Drink cases read from the chain's own price per litre and from packs the
-- name states, each confirmed by what one bottle costs in other chains.
-- METRO "0.5L ZAJECARSKO SVETLO PIVO PB" at 1.680 was one bottle; it is 20.
...
BEGIN
    ...
    -- A case is also told by the chain's price per litre, where its single
    -- bottles show that price is right, or by a pack the name states ("3+1
    -- GRATIS", "DUO", "4PACK"). METRO "0.33L ZAJECARSKO SVET.PIVO LIM" at
    -- 1.583,71 is 7,92 l at 199,97 per litre: 24 cans, not the 21 the price of
    -- one can suggested. Either reading has to fit what one bottle of that
    -- brand and size costs in at least two chains, so a vodka priced per tenth
    -- of a litre stays one bottle and a liqueur "+2 čaše" is not two (V76).
    DROP TABLE IF EXISTS pg_temp.stated_pack;

    CREATE TEMP TABLE stated_pack ON COMMIT DROP AS
    WITH reliable AS (
        ...
        WHERE (target_retailer_id IS NULL OR product.retailer_id = target_retailer_id)
          ...
          AND product.quantity_value > 0
          AND product.base_unit IN ('g', 'ml')
          AND offer.regular_price > 0
          AND offer.unit_price > 0
        GROUP BY product.retailer_id
        HAVING AVG(CASE
                       WHEN ABS(offer.regular_price
                                - offer.unit_price * product.quantity_value / 1000)
                            <= 0.02 * offer.regular_price
                           THEN 1
                       ELSE 0
                   END) >= 0.9
    ), drink AS (
        SELECT product.id,
               MIN(assignment.product_category_id) AS product_category_id
        FROM app.retailer_product AS product
        JOIN app.retailer_product_category AS assignment
          ON assignment.retailer_product_id = product.id
        JOIN app.product_category AS category
          ON category.id = assignment.product_category_id
        WHERE category.code IN ('BEVERAGES', 'WATER', 'BEER', 'WINE', 'SPIRITS')
          AND product.base_unit = 'ml'
        GROUP BY product.id
    ), candidate AS (
        SELECT scoped.id,
               -- "ZAJEČARSKO" and "Zajecarsko" are one brand.
               app.product_match_brand_key(product.brand) AS brand_key,
               drink.product_category_id,
               scoped.package_size,
               ratio.litres * 1000 / scoped.package_size AS unit_count,
               ratio.price,
               CASE
                   WHEN scoped.name ~ '[0-9]+\s*\+\s*[0-9]+'
                       THEN SUBSTRING(scoped.name FROM '([0-9]+)\s*\+\s*[0-9]+')::INTEGER
                            + SUBSTRING(scoped.name FROM '[0-9]+\s*\+\s*([0-9]+)')::INTEGER
                   WHEN scoped.name ~* '\mduo\M' THEN 2
                   WHEN scoped.name ~* 'fourpack' THEN 4
                   WHEN scoped.name ~* 'sixpack' THEN 6
                   WHEN scoped.name ~* '[0-9]+\s*-?\s*pack'
                       THEN SUBSTRING(LOWER(scoped.name) FROM '([0-9]+)\s*-?\s*pack')::INTEGER
               END AS named_pack
        FROM package_scoped AS scoped
        JOIN drink ON drink.id = scoped.id
        JOIN app.retailer_product AS product ON product.id = scoped.id
        JOIN reliable ON reliable.retailer_id = scoped.retailer_id
        CROSS JOIN LATERAL (
            SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (
                       ORDER BY offer.regular_price / offer.unit_price
                   ) AS litres,
                   PERCENTILE_CONT(0.5) WITHIN GROUP (
                       ORDER BY offer.regular_price
                   ) AS price
            FROM app.current_price_offer AS offer
            WHERE offer.retailer_product_id = scoped.id
              AND offer.regular_price > 0
              AND offer.unit_price > 0
        ) AS ratio
        WHERE scoped.named_count IS NULL
          AND scoped.package_size > 0
          AND scoped.name !~* '[0-9]\s*[x×]\s*[0-9]'
          AND scoped.name !~* '\m(cca|ca|rinfuz)\M'
          AND ratio.litres IS NOT NULL
          AND app.product_match_brand_key(product.brand) <> ''
    ), single AS (
        -- One bottle as chains sell it: no pack words, and seen in two chains.
        SELECT app.product_match_brand_key(product.brand) AS brand_key,
               product.quantity_value AS piece_size,
               drink.product_category_id,
               PERCENTILE_CONT(0.5) WITHIN GROUP (
                   ORDER BY COALESCE(NULLIF(offer.regular_price, 0), offer.discounted_price)
               ) AS price
        FROM app.retailer_product AS product
        JOIN drink ON drink.id = product.id
        JOIN app.current_price_offer AS offer
          ON offer.retailer_product_id = product.id
        WHERE product.package_count = 1
          AND app.product_match_brand_key(product.brand) IN (SELECT brand_key FROM candidate)
          AND COALESCE(NULLIF(offer.regular_price, 0), offer.discounted_price) > 0
          AND product.name !~* '[0-9]\s*\+\s*[0-9]|\+\s*[0-9]|\mduo\M|pack|gratis|[0-9]/1\M|[0-9]\s*[x×]\s*[0-9]|stand\.?-var'
        GROUP BY app.product_match_brand_key(product.brand), product.quantity_value, drink.product_category_id
        HAVING COUNT(DISTINCT product.retailer_id) >= 2
    )
    SELECT candidate.id,
           CASE
               -- A stated pack decides the count; the prices only confirm it.
               -- Free bottles make each one cheaper, hence the wider band.
               WHEN candidate.named_pack IS NOT NULL THEN
                   CASE
                       WHEN candidate.named_pack BETWEEN 2 AND 48
                        AND (
                            ROUND(candidate.unit_count) = candidate.named_pack
                            OR candidate.price / candidate.named_pack
                               BETWEEN 0.35 * single.price AND 1.3 * single.price
                        )
                           THEN candidate.named_pack
                   END
               WHEN ROUND(candidate.unit_count) BETWEEN 2 AND 48
                AND ABS(candidate.unit_count - ROUND(candidate.unit_count))
                    <= 0.03 * ROUND(candidate.unit_count)
                AND candidate.price / ROUND(candidate.unit_count)
                    BETWEEN 0.5 * single.price AND 1.3 * single.price
                   THEN ROUND(candidate.unit_count)::INTEGER
           END AS case_count
    FROM candidate
    LEFT JOIN single
      ON single.brand_key = candidate.brand_key
     AND single.piece_size = candidate.package_size
     AND single.product_category_id = candidate.product_category_id;

    DELETE FROM stated_pack WHERE case_count IS NULL;
    ...
    FROM (
        SELECT scoped.id,
               scoped.named_count,
               COALESCE(
                   stated_pack.case_count,
                   package_case.case_count,
                   NULLIF(scoped.named_count, 0),
                   1
               ) AS package_count,
               CASE
                   WHEN scoped.named_count IS NOT NULL THEN scoped.quantity_value
                   ELSE scoped.package_size
                        * COALESCE(stated_pack.case_count, package_case.case_count, 1)
               END AS quantity_value
        FROM package_scoped AS scoped
        LEFT JOIN package_case ON package_case.id = scoped.id
        LEFT JOIN stated_pack ON stated_pack.id = scoped.id
    ) AS sizes
